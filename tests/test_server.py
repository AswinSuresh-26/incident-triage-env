...
def test_tasks_endpoint_lists_three_tasks():
    ...
    assert set(ids) == {"easy", "medium", "hard"}
    assert all(task["has_grader"] is True for task in data["tasks"])


def test_grade_via_task_id_only():
    r = client.post("/grade", json={"task_id": "easy"})
    assert r.status_code == 200
    data = r.json()
    assert data["task_id"] == "easy"
