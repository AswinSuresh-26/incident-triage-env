...
def test_exploration_capped_at_max():
    ...
    # Third inspect_logs: capped (0 exploration) + loop penalty (-0.10) = -0.10, clamped to MIN_SCORE (0.01)
...
def test_perfect_terminal_reward():
    ...
    assert reward == pytest.approx(0.9)  # perfect + efficiency (<=3 steps), capped at 0.8
    assert done is True
...
def test_destructive_wrong_action_penalty():
    # Restarting wrong critical service on T3 -> -0.30, clamped to MIN_SCORE (0.01)
...
def test_t2_efficiency_bonus_requires_investigation():
    ...
    assert reward == pytest.approx(0.7)  # perfect (no investigation bonus)
    assert done is True
...
def test_t2_efficiency_bonus_with_investigation():
    ...
    assert reward == pytest.approx(0.9)  # perfect + efficiency bonus, capped at 0.8
    assert done is True
...
def test_grade_clipped_to_one():
    # Stacking exploration + efficiency could exceed 1.0 — must be clipped strictly below 1.0
